function [fh,model] = vizSingleModel_IndvLeg_YoungAdults(singleModel,Y,U,lower,labels,isf)
...
viewPoints=[35,44,380,394]; %Stroke
binw=4; %Plus minus 2
...
% YA=[model{1}.Data(:,1:70) model{1}.Data(:,end-70:end)];
% % else
% %   YA=[model{1}.Data(:,450:481+60)];  
% % end
...
yl=ax.YAxis.Limits;
pp=patch([40 390 390 40],[0 0 max(aux1) max(aux1)],.7*ones(1,3),'FaceAlpha',.5,'EdgeColor','none'); %PATR - PATS 
uistack(pp,'bottom')
ax.YAxis.Limits=yl;
axis tight
yticks('auto')

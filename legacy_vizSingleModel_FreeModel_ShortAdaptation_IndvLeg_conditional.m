function [fh,model] = legacy_vizSingleModel_FreeModel_ShortAdaptation_IndvLeg_cond(singleModel,Y,U,lower)
...
elseif lower==3 %removing the mean on the data. similar analysis as PCA 
    ...
    C1=model{1}.C1; %getting C from the struture
    C1inv=pinv(C1)'; %pseudoinverse of the C (C is not a squared matrix)
    C2=model{1}.C2; %getting C from the struture
    C2inv=pinv(C2)'; %pseudoinverse of the C (C is not a squared matrix)
%     X = Y'*Cinv(:,1:3); %x= y/C getting the dynamics of the hidden states (we are using least-sqaures seee: Penrose, Roger (1956))
    X_Adapt = Y(:,1:480)'*C1inv;
    X_Post= Y(:,481:end)'*C2inv;
%     X(:,1)=ones(length(Y),1);
%     Y2= Cs(:,1:3) * X' ; % Data reconstructed with the perdetermine dynamics
    ...
    model{1}.Data=Y;
    model{1}.States=[X_Adapt; X_Post];
    model{1}.Out=[Y_Adapt; Y_Post];
    model{1}.Res=Y-model{1}.Out;
end

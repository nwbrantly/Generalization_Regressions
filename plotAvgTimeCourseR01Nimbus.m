function PlottedMean=plotAvgTimeCourseR01Nimbus(adaptDataList,params,conditions,binwidth,trialMarkerFlag,indivFlag,indivSubs,colorOrder,biofeedback,removeBiasFlag,labels,filterFlag,plotHandles,alignEnd,alignIni,alignConditions, row, col, sub)
...
for c=1:nConds
    ...
    elseif isa(conditions{c},'char') %Chenged by Marcela
        cond{c}=lower(conditions{c}(ismember(conditions{c},['A':'Z' 'a':'z' '0':'9']))); %remove non alphanumeric characters
    else
...
if nargin<16
    alignConditions=[];
else % Added by Marcela
    for c=1:length(alignConditions)
        if isa(alignConditions{c},'cell')
            alignConditions{c}=alignConditions{c}{1}(ismember(alignConditions{c}{1},['A':'Z' 'a':'z' '0':'9']));
        elseif isa(conditions{c},'char')
            alignConditions{c}=lower(alignConditions{c}(ismember(alignConditions{c},['A':'Z' 'a':'z' '0':'9']))); %remove non alphanumeric characters
        else
            error('alignConditions argument is neither a string, a cell array of strings or a cell array of cell array of strings.')
        end
    end
end

grays = [178 190 181; 128 128 128; 211 211 211; 54 69 79; 137 148 153; ...
    192 192 192; 113 121 126]./255;
...
%     poster_colors;
%     if length(labels)>=3 || length(conditions) > 3
...
for group=1:Ngroups
    for subject=1:length(adaptDataList{group})
        ...
        if removeBiasFlag==1
%             adaptData = adaptData.removeBias([],true);
            adaptData = adaptData.removeBias;
        elseif removeBiasFlag==2
            adaptData = adaptData.normalizeBias; %Change here use removeBiasAlt
        end
        ...
        for c=1:nConds
            ...
            for t=1:length(trials) %length(trials) is always 1
                ...
                elseif monoFlag
                    ...
                    dataPts=dataPts(2:end-5); %% Added by Marcela to exclude the very first stride, and the very last 5 strides
                end
                ...
                if nPoints == 0
                    ...
                    if ~isempty(alignEnd) %Aligning data to the end, too
                        if strcmpi(cond{c},'catch')
                            numPts.([cond{c}]).(['trial' num2str(t)])(s)=nPoints;
                        elseif sum(contains(alignConditions,cond{c}))~=0 || isempty(alignConditions)
                            b=alignIni(contains(alignConditions,cond{c}));
                            if b==0
                                numPts.([cond{c} 'End']).(['trial' num2str(t)])(s)=alignEnd(cc);
                            else
                                numPts.([cond{c} 'End']).(['trial' num2str(t)])(s)=alignEnd(cc)+50;
                            end
                        end
                    end
                end
                ...
                for p=1:length(params)
                    ...
                    if ~isempty(alignEnd) %Aligning data to the end too, by creating a fake condition
                        ...
                        elseif sum(contains(alignConditions,cond{c}))~=0 || isempty(alignConditions)
%                             cc=1+cc;
%                             values(group).(params{p}).([cond{c} 'End']).(['trial' num2str(t)])(subject,:)=[nan(50,1); dataPts(end-alignEnd-4:end-5,p)]; %Removes the very last 5 strides, which is what we do with removeBias and with get_Results functions for the stats
                            b=alignIni(contains(alignConditions,cond{c}));                            
                            if b==0
                                values(group).(params{p}).([cond{c} 'End']).(['trial' num2str(t)])(subject,:)=[dataPts(end-alignEnd(cc)+1:end,p)];
                            else
                                values(group).(params{p}).([cond{c} 'End']).(['trial' num2str(t)])(subject,:)=[nan(50,1); dataPts(end-alignEnd(cc)+1:end,p)];
                            end
                            cc=1+cc;
                        end
                    end
...
se=struct([]);
indiv=struct([]);
if ~isempty(alignEnd) && isempty(alignConditions) 
    ...
    cond=newCond;
    nConds=length(cond);    
elseif ~isempty(alignEnd) && ~isempty(alignConditions) 
    newCond=cell(length(cond)+length(alignConditions),1);
    cont=1;
    for i=1:length(cond)
        newCond{cont}=cond{i};
        if sum(contains(alignConditions,cond{i}))==1
            cont=cont+1;
            newCond{cont}=[cond{i} 'End'];
        end
        cont=cont+1;
    end    
    catchindx=find(strcmpi(newCond,'catchEnd'));
        ...
        for t=1:length(fields(values(group).(params{p}).(cond{c})))
            ...
            for p=1:length(params)
                ...
                elseif ~isempty(alignIni) && strcmp(cond{c}(end-2:end), 'End')
                    ...
                    if isempty(alignConditions)
                        x=Xstart:Xstart+alignIni-1;
                        y=y(1:length(x));
                        E=E(1:length(x));
                        condLength=length(y);
                    elseif sum(contains(alignConditions,cond{c}))==1
                        b=contains(alignConditions,cond{c});
%                         alignIni(b)
                        x=Xstart:Xstart+alignIni(b)-1;
                        y=y(1:length(x));
                        E=E(1:length(x));
                        condLength=length(y);
                    else
                ...
                if indivFlag %plotting all individual subjects
                    ...
                    for s=1:length(subsToPlot)
                        ...
                        Li{group}(s)=plot(x,y_indiv(1:length(x)),'color',[colorOrder(group, :), 0.6],'Tag',id);
                    ...
                    %in the group
                    if length(adaptDataList{group})>1
                        Li{group}(length(subsToPlot)+1)=plot(x,y,'o','MarkerSize',5,'MarkerEdgeColor',[0 0 0],'MarkerFaceColor',colorOrder(group, :));     %[0.7 0.7 0.7].^group
...
%                 end
%                 
                % Marcela commented next line
                %                 xticks=lineX+diff([lineX Xstart])./2;
                
                
                % Commented by Marcela
                line([lineX; lineX],ylim,'color','k')
                xticks=lineX+diff([lineX Xstart])./2;
        ...
        elseif ~isempty(alignEnd) && c<length(cond)
            if contains(cond{c+1},'End')
                continue
            else 
                lineX(end+1)=Xstart-0.5;
            end

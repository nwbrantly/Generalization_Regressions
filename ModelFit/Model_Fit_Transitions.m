...
temp=diff([zeros(40,1); velR;zeros(100,1)]-[zeros(40,1); velL;zeros(100,1)]);
temp2=find(temp~=0);
transitions=temp2-1;
% C=[C1 C3];
% C=Cnew;
...
speed=[zeros(40,1); velR]-[zeros(40,1); velL];
for i=1:length(transitions)+1
    if i==1
        Xasym=[X2asym(1:transitions(i)-1,:); nan(1,3)];
        speed2=[speed(1:transitions(i)-1,:); nan(1,1)];
        tt(i)=transitions(i)+i;
    elseif i<=length(transitions)
        Xasym=[Xasym; X2asym(transitions(i-1):transitions(i)-1,:);nan(1,3)];
        speed2=[speed2;speed(transitions(i-1):transitions(i)-1,:);nan(1,1)];
         tt(i)=transitions(i)+i;
    else
        Xasym=[Xasym; X2asym(transitions(i-1):end,:)];
        speed2=[speed2;speed(transitions(i-1):end,:)];
    end
...
for i=2:2:length(transitions)
%     pp=patch([transitions(i-1) transitions(i) transitions(i) transitions(i-1)],[-0.5 -0.5 1 1],.7*ones(1,3),'FaceAlpha',.2,'EdgeColor','none');
    pp=patch([tt(i-1) tt(i) tt(i) tt(i-1)],[-0.5 -0.5 1 1],.7*ones(1,3),'FaceAlpha',.2,'EdgeColor','none');
    uistack(pp,'bottom')
end
%  plot(speed2,'LineWidth',2);
legend('Split','AutoUpdate','off')
axis tight
ylabel('X_{baseline}')
% plot(speed2,'LineWidth',2);
yline(0)
...
legacy_vizSingleModel_FreeModel_Transitions(model,Yasym',Uf)
